...
This document is about the **compiler** — the program that turns
`specs/noctua.toml` into every artifact under `system/`. If you only want to
*use* the colours, the [README](../README.md) is the whole story and you do not
need any of this.

It is here because the colours are only worth trusting if the method is, and
the method is more interesting than the output.
...
- [The gamut boundary, solved exactly](#the-gamut-boundary-solved-exactly)
- [Palettes come in a grid](#palettes-come-in-a-grid)
- [Every context an application needs](#every-context-an-application-needs)
- [What exists](#what-exists)
- [What the quality gates found, and could not fix](#what-the-quality-gates-found-and-could-not-fix)
- [Does the model actually hold?](#does-the-model-actually-hold)
- [The command surface](#the-command-surface)
- [Using `noctua-core` directly](#using-noctua-core-directly)
...
**The repository versions the curves, not the colours.** No hand-picked hex
value exists anywhere in the source; there is a lint that fails the build if one
appears. Every colour is computed, checked, and regenerated on demand.
...
## The gamut boundary, solved exactly

Relative chroma is a fraction of "the most this gamut can show at this lightness
and hue". That number has to be *exact*, and computed for every step of every
ramp in every palette — otherwise the whole model rests on an approximation.

The usual answer is a binary search: guess a chroma, convert, check whether it
is in gamut, halve the interval. It converges, it is slow, and it is only ever
approximately right.

The boundary is instead solved **analytically**. For a fixed lightness and hue,
the OKLCH-to-linear-RGB transform is a cubic in chroma per channel, so the
largest displayable chroma is the smallest positive real root across the three
channels' cubics. `noctua-core` derives every colour matrix at compile time from
published chromaticities, solves the cubics in closed form, and clips only at
the very end to absorb floating-point error.

`map_into_gamut` uses the same machinery: it holds lightness and hue fixed and
reduces chroma until the colour fits — CSS Color 4 chroma reduction, never
per-channel clipping, which would shift hue. It returns how much chroma it had
to give up, and that number is what the site's "distance to the gamut boundary"
...
**1,767 names in the semantic contract, covering 349 subjects** — each subject
emitted as the fill, its hover, a tinted background, a border, and a foreground
to put on the fill. Grouped in `specs/noctua.toml` by subject, because that is
how a name gets looked up:
...
**Ten of these have a hue; the rest are aliases.** A family costs a full
twelve-role ramp in every theme, mode and gamut; an alias costs five `var()`
references. The wheel has room for ten hues and not for three hundred and fifty,
so a context earns a family only when its meaning has to be told apart from
every other *without a legend* — `waiting` beside `active` in a job list,
`urgent` beside `danger` in an alert feed.

The alias layer is the same in every palette, so it is emitted **once**, into
`system/css/contexts.css` and the top of `system/json/palette.json`, with only
what a `[themes.<name>.semantic]` block overrides written per theme. Repeated
across thirty-nine palettes it was 97 KB of every 225 KB stylesheet.
...
One command compiles the spec into a committed `system/`; one command gates it.

| | |
|---|---|
| Oklab / OKLCH / XYZ / linear + encoded RGB | all matrices derived at compile time from published chromaticities |
| Gamut boundary | exact, solved analytically |
...
| Palettes | **39**, generated as a grid: 13 accent hues × 3 saturations, from 18 lines of specification |
| Contexts | **1,767 names** over 349 subjects and 10 families, plus two ordered scales, three neutral temperatures and a translucency ladder |
...
| Documentation site | static, generated from `system/`, dogfooding its own tokens |
...
`system/` sync check that fails if a generated file was edited by hand, and
**48,441 palette checks** across every gate (`cargo xtask check` prints the
current count) — the contrast pairs and the colour-vision margins are generated
from the semantic contract, so adding a context extends the matrix rather than
leaving a hole nobody notices.

---

## What the quality gates found, and could not fix
...
six-family semantic set is **0.0163** — under one just-noticeable difference,
and there are now ten families.

So the gate reports margins and warns; it fails only when two colours are
literally the same. `system/reports/colour-vision.md` publishes every number.
...
The same measurement decided the categorical scale. Eight generated colours
...
there is: the same authored token is a different colour in a wider gamut, and
...
The importer refuses to overclaim. A ramp with fewer colours than the model has
parameters is reported as **weakly constrained**, because a near-zero residual
on three colours is arithmetic rather than evidence.

---

## The command surface

Six verbs, and that is a constraint rather than a description: new capability
goes behind an existing verb or it does not ship. `just <verb>` is a one-word
alias for each; neither is required.

| Command | Does |
|---|---|
| `cargo xtask build` | Compile the spec into `target/system/` — a gitignored scratch tree — and render the site |
| `cargo xtask build --system` | The same, writing the published `system/` |
| `cargo xtask check` | Validate the spec, run every gate, verify `system/`, then fmt, lints and tests |
| `cargo xtask dev` | Watch the spec and the site, rebuild, serve with live reload |
| `cargo xtask export` | Copy `system/` into every consumer registered in the spec |
| `cargo xtask import <source>` | Fit an existing palette back to spec parameters |
| `cargo xtask release <version>` | Prepare a colour-system version; committing is left to a human |
| `cargo xtask release --tool <version>` | The same for the compiler's own version |

`cargo xtask check --colors-only` skips fmt, lints and tests for a fast loop
while tuning colours. `--dry-run` on `export` and `release` reports what would
happen without doing it.

**Why `build` does not write `system/` by default.** `system/` is the published
product — people link it by CDN URL, pin it as a Nix `src`, vendor it as a
submodule. It used to be the same directory every build wrote to, which meant
trying a hue out rewrote the shipped colours, and the only thing between that
and a commit was noticing a 250-file diff. Publishing is now a typed intent.

`import` reads colour literals out of CSS, SCSS, QML, JSON or plain text, groups
...
palette authored for P3 has colours sRGB cannot show, and `cr` cannot exceed 1.

---

## Using `noctua-core` directly

`noctua-core` is the colour math, and it has **no workspace dependencies** —
it knows nothing about the spec format or the output targets, and never will.

```bash
cargo doc --open -p noctua-core
```

### Finding the gamut boundary
...
colour fits, clipping only at the very end to absorb floating-point error. It
returns how much chroma it had to give up, which is what a "distance to the
gamut boundary" readout is built from.

### Checking a pair for contrast and colour-vision safety
...
colours sit ΔE 0.260 apart to normal vision and **0.009** apart under
...
every deficiency. That is the rule the semantic pairs are built on.
...
| Where | What |
|---|---|
| `/` | The product: what this is, how to install it, the full palette browser with click-to-copy in five formats, every context painted from its own token, both ordered scales, the translucency ladder, and a contrast matrix **measured live in your browser** |
| `/how-it-works.html` | This document, rendered: the two ideas, the gamut boundary, the gates and their measured limits |
...
| `*.pt.html` | The same pages in Brazilian Portuguese |
...
`system/json/themes/<name>.json`, fetching a palette's stylesheet the first time
it is chosen. Rendering all thirty-nine would be a 2.4 MB page with seventeen
thousand nodes.

The site is a **consumer**, not a second opinion: it reads
`system/json/palette.json` and paints itself with `system/css/`, so it cannot
show you a colour the compiler did not emit.

---

## Where to go next

- [`AGENTS.md`](../AGENTS.md) — the operating manual: the repository map, the
  invariants, and the gotchas that have already cost debugging time here.
- [`CONTRIBUTING.md`](../CONTRIBUTING.md) — how to work on this repository.
- [`specs/noctua.toml`](../specs/noctua.toml) — the specification itself, which
  is more heavily commented than this document is long.
