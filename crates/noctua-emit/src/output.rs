//! Writing `system/`, and verifying it is still what the spec says.
//!
...
/// Everything that belongs in `system/`, including the manifest.
#[must_use]
pub fn artifacts(palette: &Palette, spec_path: &str, spec_text: &str) -> Vec<EmittedFile> {
    ...
    let manifest = manifest(&palette.identity, spec_path, spec_text, &files);
...
/// A way in which `system/` disagrees with the spec.
#[derive(Debug, Clone, PartialEq, Eq)]
pub enum Drift {
    /// The file exists but its contents differ.
    ///
    /// Two causes, and the message must not presume either. Before `build`
    /// wrote a scratch tree this was almost always a hand-edit, because any
    /// build would have refreshed the file. Now the ordinary cause is the
    /// opposite: the spec changed and nobody ran `build --system`.
    Changed(String),
...
impl std::fmt::Display for Drift {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            Self::Changed(path) => write!(f, "system/{path} does not match the spec"),
            Self::Missing(path) => write!(f, "system/{path} is missing"),
            Self::Stale(path) => write!(f, "system/{path} is no longer generated"),
        }
    }
}

/// Checks `system/` against what the spec would produce right now.
///
/// # Errors
///
...
/// Byproducts of *using* what is in `system/`, rather than of generating it.
///
...
pub fn explain(drift: &[Drift]) -> String {
    ...
    let mut out = format!(
        "system/ is out of sync with the spec ({} file{}):\n",
        drift.len(),
    ...
    out.push_str(
        "\nEither the spec changed and system/ was not republished — run \
         `cargo xtask build --system` — or a generated file was edited by hand, \
         which is never the way to change a colour. Edit specs/noctua.toml.",
    );
    out
}
...
mod tests {
    ...
    impl Scratch {
        fn new(name: &str) -> Self {
            let path = std::env::temp_dir().join(format!("noctua-output-{name}"));
    ...
    fn a_hand_edited_file_is_caught() {
        ...
        // Both causes must be named. A reader who hit this by forgetting to
        // republish is far more common than one who hand-edited, and a message
        // that accuses them of the wrong thing sends them looking for an edit
        // that does not exist.
        let message = explain(&drift);
        assert!(message.contains("edited by hand"), "{message}");
        assert!(message.contains("was not republished"), "{message}");
        assert!(message.contains("cargo xtask build --system"), "{message}");
    }
    ...
    /// Compiling the generated crate must not make `system/` look dirty.
    #[test]
    fn build_output_from_the_generated_crate_is_not_mistaken_for_drift() {
        ...
        // What `cargo build` inside system/rust leaves behind.
